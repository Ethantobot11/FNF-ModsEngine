...
class StageEditorState extends MusicBeatState
{
	...
	var spriteList_box:FlxUITabMenu;
	...
	function screenUI()
	{
		...
		spriteList_box = new FlxUITabMenu(25, 40, 250, 200, ['Sprite List']);
		...
		focusRadioGroup = new FlxUIRadioGroup(targetTxt.x, FlxG.height - 24, ['dad', 'boyfriend', 'gf'], 10, 0, true);
		...
		lowQualityCheckbox = new FlxUICheckBox(FlxG.width - 240, FlxG.height - 36, 'Can see Low Quality Sprites?', 90);
		...
		highQualityCheckbox = new FlxUICheckBox(FlxG.width - 120, FlxG.height - 36, 'Can see High Quality Sprites?', 90);
	...
	function addSpriteListBox()
	{
		...
		spriteListRadioGroup = new FlxUIRadioGroup(10, 10, [], 25, 18, false, 200);
		...
		var buttonMoveUp:FlxButton = new FlxButton(buttonX, buttonY, 'Move Up', function()
		{
		...
		var buttonMoveDown:FlxButton = new FlxButton(buttonX, buttonY + 30, 'Move Down', function()
		{
			var selected:Int = spriteListRadioGroup.checked;
			if(selected < 0) return;
		...
		var buttonCreate:FlxButton = new FlxButton(buttonX, buttonY + 60, 'New', function() createPopup.visible = createPopup.active = true);
		...
		var buttonDuplicate:FlxButton = new FlxButton(buttonX, buttonY + 90, 'Duplicate', function()
		{
			var selected:Int = spriteListRadioGroup.checked;
			if(selected < 0) return;
		...
		var buttonDelete:FlxButton = new FlxButton(buttonX, buttonY + 120, 'Delete', function()
		{
			var selected:Int = spriteListRadioGroup.checked;
			if(selected < 0) return;
	...
	function editorUI()
	{
		UI_box = new FlxUITabMenu(FlxG.width - 225, 10, 200, 400, ['Meta', 'Data', 'Object']);
		UI_box.cameras = [camHUD];
		UI_box.scrollFactor.set();
		add(UI_box);
		UI_box.selectedName = 'Data';

		UI_stagebox = new FlxUITabMenu(FlxG.width - 275, 25, 250, 100, ['Stage']);
	...
	function addDataTab()
	{
		...
		objY += 30;
		hideGirlfriendCheckbox = new FlxUICheckBox(objX, objY, 'Hide Girlfriend?', 100);
	...
	function addObjectTab()
	{
		...
		alphaStepper = new FlxUINumericStepper(objX, objY, 0.1, 1, 0, 1, 2, true);
		...
		antialiasingCheckbox = new FlxUICheckBox(objX + 90, objY, 'Anti-Aliasing', 80);
		...
		objY += 25;
		flipXCheckBox = new FlxUICheckBox(objX, objY, 'Flip X', 60);
		flipXCheckBox.onClick = updateFlip;
		flipYCheckBox = new FlxUICheckBox(objX + 90, objY, 'Flip Y', 60);
		...
		lowQualityCheckbox = new FlxUICheckBox(objX, objY, 'Low Quality', 70);
		highQualityCheckbox = new FlxUICheckBox(objX + 90, objY, 'High Quality', 70);
	...
	function addMetaTab()
	{
		...
		var openPreloadButton:FlxButton = new FlxButton(objX, objY, 'Preload List', function() {
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxUIInputText.focusOn != null) return;
		...
		if(
			#if FLX_DEBUG
			FlxG.keys.justPressed.F3
			#else
			FlxG.keys.justPressed.F2
			#end
			|| (touchPad.buttonS.justPressed && !touchPad.buttonF.justPressed))
		{
		...
		if(fullPath != null)
		{
			...
			if(fullPath.startsWith(exePath))
			{
				...
				if((fullPath.startsWith('assets/') #if MODS_ALLOWED || fullPath.startsWith('mods/') #end) && fullPath.contains('/images/'))
				{
					...
					//trace('Inside Mods Engine Folder');
					return;
				}
			}
			...
			openSubState(new BasePrompt(480, 160, 'This file is not inside Mods Engine.', function(state:BasePrompt)
			{
				...
				var btnY = 390;
				var btn:FlxButton = new FlxButton(0, btnY, 'OK', function() {
				...
				var btn:FlxButton = new FlxButton(0, btnY, 'Cancel', function()
				{
					_makeNewSprite = null;
					state.close();
				});
...
class StageEditorAnimationSubstate extends MusicBeatSubstate {
	...
	public function new()
	{
		...
		UI_animationbox = new FlxUITabMenu(FlxG.width - 320, 20, 300, 250, ['Animations']);
	...
	function addAnimationsUI()
	{
		...
		animationLoopCheckBox = new FlxUICheckBox(animationNameInputText.x + 170, animationNameInputText.y - 1, 'Should it Loop?', 100);
		...
		var initAnimButton:FlxButton = new FlxButton(160, animationDropDown.y, 'Main Animation', function() {
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.ESCAPE
		#if android
		|| FlxG.android.justReleased.BACK || touchPad.buttonB.justPressed
		#else
