...
class PreloadListSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var btn:FlxButton = new FlxButton(bg.x + bg.width - 40, bg.y, 'X', close, 40);
		btn.cameras = cameras;
		add(btn);
		...
		removeButton = new FlxButton(0, 0, 'X', function()
		{
		...
		hqCheckBox = new FlxUICheckBox(lqCheckBox.x, lqCheckBox.y + 22, 'High Qual.', 0, updateFilters);
		smCheckBox = new FlxUICheckBox(hqCheckBox.x, hqCheckBox.y + 22, 'Story Mode', 0, updateFilters);
		...
		radioGrp = new FlxUIRadioGroup(bg.x + 60, bg.y + 80, preloadListKeys, 25, 15, false, 280);
		...
		function addToList(path:Path, isFolder:Bool)
		{
			...
			else showOutput('File is not inside Mods Engine\'s folder!', true);
		}

		var loadFileBtn:FlxButton = new FlxButton(0, bg.y + bg.height - 40, 'Load File', function()
		{
		...
		var loadFolderBtn:FlxButton = new FlxButton(0, bg.y + bg.height - 40, 'Load Folder', function()
		{
			if(!fileDialog.completed) return;
		...
		var saveBtn:FlxButton = new FlxButton(0, bg.y + bg.height - 40, 'Save', function()
		{
			if(!fileDialog.completed) return;
